...
local jsn_upd = "https://gitlab.com/snippets/2087435/raw"
...
--tags
local imgIntGameRUS = {[[Приветствуем Вас, дорогой пользователь! В данном проекте есть вспомогательные функции для..
..Вашей игры в SA:MP.
Поможем Вам разобраться в zuwi - сверху видите вкладки, такие как, "Персонаж", "Транспорт", ..
.. "Оружие" и другие.
...
*Разное - эта вкладка отвечает за прочие функции, они могут быть как "читы", или..
.. чтобы, "глазу было приятно".
Также есть подвкладка "Телепорты", там Вы сможете телепортироваться на любое место, ..
.. которое есть в списке;
*Визуалы - эта вкладка отвечает за любую отрисовку в zuwi. То есть вне игровые отрисовки, ..
.. к примеру, открыты ли транспортные средства или же нет и тому подобное;
...
*Помощь - эта вкладка отвечает за Вашу помощь. Если Вы забыли что-либо, либо же..
.. не понимаете что-то, можете открывать данную вкладку и там Вы скорее всего..
.. найдете решение Вашей проблемы;]],
[[Это краткая помощь. Надеемся на то, что Вы разберетесь в данном творении.
Помните, что за "читы" могут выдать наказания, за которые мы не несем ответственности, ..
используйте на свой СТРАХ И РИСК!
Будем очень благодарны за любую помощь! С любовью ZUWI Project!]]}
...
*Visual - this tab is responsible for any drawing in zuwi. That is, outside the game renders, ..
...
We will be very grateful for any help! With love ZUWI Project!]]}
--others
...
function main()
	...
	sampRegisterChatCommand('zdate', function() sampAddChatMessage(os.date(array.lang_chat[0] and tag..'Сегодняшняя дата: {F9D82F}%d.%m.%Y' or tag..'Todays date: {F9D82F}%d.%m.%Y'), colors.chat.main) end)
	...
	sampRegisterChatCommand('zcoord', function()
		if not isPlayerDead(PLAYER_PED) then
			...
			sampAddChatMessage(tag..(array.lang_chat[0] and '{B31A06}Ошибка: {888EA0}Ваш игрок мертв/не существует' or '{B31A06}Error: {888EA0}You are player is dead/not playing'), colors.chat.main)
		end
	end)
	...
	sampRegisterChatCommand('zrepair', function(arg)
		if isPlayerPlaying(PLAYER_PED) and not isPlayerDead(PLAYER_PED) and isCharInAnyCar(PLAYER_PED) then
			...
			if #arg == 0 then
				fixCar(myCar)
				if getServer('revent') then sampAddChatMessage(' Вы починили транспорт!', 16113331) else printStringNow('Fixed vehicle ~g~1000 HP', 1000) end
			elseif arg == 'max' then
				fixCar(myCar); setCarHealth(myCar, 1410065408)
				if getServer('revent') then sampAddChatMessage(' Вы починили транспорт!', 16113331) else printStringNow('Fixed vehicle ~g~1410065408 HP', 1000) end
			else
				fixCar(myCar); setCarHealth(myCar, arg)
				if getServer('revent') then sampAddChatMessage(' Вы починили транспорт!', 16113331) else printStringNow('Fixed vehicle ~g~'..arg..' HP', 1000) end
			end
		else
			sampAddChatMessage(tag..(array.lang_chat[0] and '{B31A06}Ошибка: {888EA0}Ваш игрок мертв/не существует или не в транспорте' or '{B31A06}Error: {888EA0}You are player is dead/not playing or is not in vehicle'), colors.chat.main)
		end
	end)
	sampRegisterChatCommand('ztime', function(param)
		...
		if hour ~= nil and hour >= 0 and hour <= 23 then
			...
			sampAddChatMessage(tag..(array.lang_chat[0] and 'Время изменено на {F9D82F}'..hour or 'Time change to {F9D82F}'..hour), colors.chat.main)
		else
			patch_samp_time_set(false)
			hour = nil
			sampAddChatMessage(tag..(array.lang_chat[0] and '{B31A06}Ошибка: {888EA0}Не найдено время. Используйте: {0984d2}/ztime 0-23' or '{B31A06}Error: {888EA0}Time not found. Use: {0984d2}/ztime 0-23{888EA0})'), colors.chat.main)
		end
		setTimeOfDay(hour, 0)
	end)
	sampRegisterChatCommand('zweather', function(param)
		...
		if weather ~= nil and weather >= 0 and weather <= 45 then
			...
			sampAddChatMessage(tag..(array.lang_chat[0] and '{B31A06}Ошибка: {888EA0}Погода не найдена. Используйте: {0984d2}/zweather 0-45' or '{B31A06}Error: {888EA0}Weather not found. Use: {0984d2}/zweather 0-45'), colors.chat.main)
		end
	end)
	...
	sampRegisterChatCommand('zsetmark', function(arg)
		if #arg == 0 then
			...
			sampAddChatMessage(tag..(array.lang_chat[0] and 'Интерьер: {F9D82F}'..intMark or 'Interior: {F9D82F}'..intMark), colors.chat.main)
		else
	...
	sampRegisterChatCommand('ztpmark', function()
		if setmark then
			...
			sampAddChatMessage(tag..(array.lang_chat[0] and '{B31A06}Ошибка: {888EA0}Метка не создана' or '{B31A06}Error: {888EA0}Mark not create'), colors.chat.main)
		end
	end)
	...
	sampRegisterChatCommand('zchecktime', function() local time = getTime(0) sampAddChatMessage(tag..(array.lang_chat[0] and 'Точное время по МСК: {F9D82F}' or 'Exact time to MSK: {F9D82F}')..os.date('%H{888EA0}:{F9D82F}%M{888EA0}:{F9D82F}%S', time), colors.chat.main) end)
	sampRegisterChatCommand('zreload', function() thisScript():reload() end)
	sampRegisterChatCommand('zfps', function() local ifps = string.format('%d', mem.getfloat(0xB7CB50, 4, false)) sampAddChatMessage(tag..(array.lang_chat[0] and 'Сейчас FPS: {F9D82F}'..ifps or 'FPS now: {F9D82F}'..ifps), colors.chat.main) end)
...
local mainFrame = imgui.OnFrame(
    ...
    function(self)
        ...
        if checkfuncs.main.enabled and not isSampfuncsConsoleActive() and not sampIsChatInputActive() and not sampIsDialogActive() and not isPauseMenuActive() then
			...
			imgui.Text('\tzuwi | Version: ' .. version_script.. ' | Uptime: '..ltime.hours..':'..ltime.minutes..':'..ltime.seconds)
			imgui.SameLine()
			imgui.Indent(750)
			...
			imgui.IntSpacing(5)
			imgui.CenterTextColored(imgui.ImVec4(imgui.GetStyle().Colors[imgui.Col.Text]), array.lang_menu[0] and u8'Быстрое меню' or 'Quick menu')
			if imgui.Button(array.lang_menu[0] and u8'Выгрузка' or 'Unload', imgui.ImVec2(100, 0)) then thisScript():unload() end
			if not array.lang_menu[0] then if imgui.Button(u8'Сменить язык', imgui.ImVec2(100, 0)) then array.lang[0] = 1 end else if imgui.Button('Change language', imgui.ImVec2(100, 0)) then array.lang[0] = 2 end end
			...
			imgui.Hint('##ft_save', array.lang_menu[0] and u8'Сохранить настройки' or 'Save settings')
			imgui.SameLine()
			if imgui.Button(fa.ICON_DOWNLOAD) then autoupdate(jsn_upd, tag, url_upd) end
			imgui.Hint('##ft_autoupdate', array.lang_menu[0] and u8'Проверить обновления' or 'Check updates')
			imgui.EndChild()
			imgui.SameLine()
            if act1 == 1 then
                imgui.BeginChild('##actor', imgui.ImVec2(sw-140, sh-40), true)
				imgui.ToggleButton(array.typeToggle[0], array.lang_menu[0] and u8'Бесконечная выносливость (бег)' or 'Infinity stamina (run)', array.show_imgui_infRun)
				imgui.ToggleButton(array.typeToggle[0], array.lang_menu[0] and u8'Бесконечная выносливость (плавание)' or 'Infinity stamina (swim)', array.show_imgui_infSwim)
				imgui.ToggleButton(array.typeToggle[0], array.lang_menu[0] and u8'Бесконечный кислород' or 'Infinity oxygen', array.show_imgui_infOxygen)
				imgui.ToggleButton(array.typeToggle[0], array.lang_menu[0] and u8'Мега прыжок' or 'Mega jump', array.show_imgui_megajumpActor)
				imgui.ToggleButton(array.typeToggle[0], array.lang_menu[0] and u8'Быстрый бег' or 'Fast sprint', array.show_imgui_fastsprint)
				imgui.ToggleButton(array.typeToggle[0], array.lang_menu[0] and u8'Без падений' or 'No fall', array.show_imgui_nofall)
				imgui.ToggleButton(array.typeToggle[0], array.lang_menu[0] and u8'Разморозить' or 'Unfreeze', array.show_imgui_unfreeze)
				...
				imgui.ToggleButton(array.typeToggle[0], array.lang_menu[0] and u8'Суицид' or 'Suicide', array.show_imgui_suicideActor)
				...
				imgui.ToggleButton(array.typeToggle[0], array.lang_menu[0] and u8'Бесконечное здоровье' or 'GM', array.show_imgui_gmActor)
				...
				imgui.ToggleButton(array.typeToggle[0], array.lang_menu[0] and u8'Невидимка' or 'Invise', array.ainvise)
				imgui.SameLine()
				imgui.Text(fa.ICON_COG)
				if imgui.IsItemClicked() then
					imgui.OpenPopup(array.lang_menu[0] and u8'Настройки Invise' or 'Settings Invise')
				end
				if imgui.BeginPopup(array.lang_menu[0] and u8'Настройки Invise' or 'Settings Invise') then
					imgui.RadioButtonIntPtr(array.lang_menu[0] and u8'Под землей' or 'Underground', array.ainviseInt, 1)
					imgui.RadioButtonIntPtr(array.lang_menu[0] and u8'В небе' or 'In the sky', array.ainviseInt, 2)
					imgui.EndPopup()
				end
				imgui.ToggleButton(array.typeToggle[0], array.lang_menu[0] and u8'Быстро останавливаться' or 'Stop quickly', array.stopQuick)
                imgui.EndChild()
            elseif act1 == 2 then
				...
				if imgui.BeginPopup(array.lang_menu[0] and u8'Настройки Speedhack' or 'Settings Speedhack') then
					...
					imgui.SliderFloat(array.lang_menu[0] and u8'Максимальная скорость' or 'Max speed', array.SpeedHackMaxSpeed, 80, 300, '%.1f', 0.5)
					imgui.SliderInt(array.lang_menu[0] and u8'Плавность' or 'Smooth', array.SpeedHackSmooth, 5, 150)
					imgui.PopItemWidth()
					imgui.EndPopup()
				end
				imgui.ToggleButton(array.typeToggle[0], array.lang_menu[0] and u8'Переворот на 180' or 'Flip 180', array.show_imgui_flip180)
				...
				imgui.ToggleButton(array.typeToggle[0], array.lang_menu[0] and u8'Переворот на колеса' or 'Flip on wheels', array.show_imgui_flipOnWheels)
				...
				imgui.ToggleButton(array.typeToggle[0], array.lang_menu[0] and u8'Прыжочек' or 'Hop', array.show_imgui_hopVeh)
				...
				imgui.ToggleButton(array.typeToggle[0], array.lang_menu[0] and u8'Взрыв транспорта' or 'Boom vehicle', array.show_imgui_suicideVeh)
				...
				imgui.ToggleButton(array.typeToggle[0], array.lang_menu[0] and u8'Быстрый выход' or 'Fast exit', array.show_imgui_fastexit)
				...
				imgui.ToggleButton(array.typeToggle[0], array.lang_menu[0] and u8'Починить колеса' or 'Restore wheels', array.show_imgui_fixWheels)
				...
				imgui.ToggleButton(array.typeToggle[0], array.lang_menu[0] and u8'Мега BMX прыжок' or 'Mega BMX jump', array.show_imgui_megajumpBMX)
				imgui.ToggleButton(array.typeToggle[0], array.lang_menu[0] and u8'Идеальная езда' or 'Perfect handling', array.show_imgui_perfectHandling)
				imgui.ToggleButton(array.typeToggle[0], array.lang_menu[0] and u8'У всего транспорта нитро' or 'All cars have nitro', array.show_imgui_allCarsNitro)
				imgui.ToggleButton(array.typeToggle[0], array.lang_menu[0] and u8'Танк мод' or 'Tank mode', array.show_imgui_tankMode)
				imgui.ToggleButton(array.typeToggle[0], array.lang_menu[0] and u8'Езда по воде' or 'Drive on water', array.show_imgui_driveOnWater)
				imgui.ToggleButton(array.typeToggle[0], array.lang_menu[0] and u8'Починить транспорт' or 'Restore health', array.show_imgui_restHealthVeh)
				...
				imgui.ToggleButton(array.typeToggle[0], array.lang_menu[0] and u8'Двигатель включен' or 'Engine on', array.show_imgui_engineOnVeh)
				imgui.NextColumn()
				imgui.ToggleButton(array.typeToggle[0], array.lang_menu[0] and u8'Не взрывается при перевороте' or 'Does not explode on coup', array.antiboom_upside)
				...
				imgui.ToggleButton(array.typeToggle[0], array.lang_menu[0] and u8'Бесконечное здоровье' or 'GM', array.show_imgui_gmVeh)
				...
				if imgui.BeginPopup(array.lang_menu[0] and u8'Настройки GMVeh' or 'Settings GMVeh') then
					imgui.ToggleButton(array.typeToggle[0], array.lang_menu[0] and u8'Обычный' or 'Default', array.show_imgui_gmVehDefault)
					imgui.ToggleButton(array.typeToggle[0], array.lang_menu[0] and u8'Колеса' or 'Wheels', array.show_imgui_gmVehWheels)
					imgui.EndPopup()
				end
				imgui.ToggleButton(array.typeToggle[0], array.lang_menu[0] and u8'Невидимка' or 'Invise', array.vinvise)
				imgui.SameLine()
				imgui.Text(fa.ICON_COG)
				if imgui.IsItemClicked() then
					imgui.OpenPopup(array.lang_menu[0] and u8'Настройки VInvise' or 'Settings VInvise')
				end
				if imgui.BeginPopup(array.lang_menu[0] and u8'Настройки VInvise' or 'Settings VInvise') then
					imgui.RadioButtonIntPtr(array.lang_menu[0] and u8'Под землей' or 'Underground', array.vinviseInt, 1)
					imgui.RadioButtonIntPtr(array.lang_menu[0] and u8'В небе' or 'In the sky', array.vinviseInt, 2)
					imgui.EndPopup()
				end
                imgui.EndChild()
            elseif act1 == 3 then
                imgui.BeginChild('##weapon', imgui.ImVec2(sw-140, sh-40), true)
				imgui.ToggleButton(array.typeToggle[0], array.lang_menu[0] and u8'Бесконечные патроны' or 'Infinity ammo', array.show_imgui_infAmmo)
				imgui.ToggleButton(array.typeToggle[0], array.lang_menu[0] and u8'Без перезарядки' or 'No reload', array.show_imgui_noreload)
				imgui.ToggleButton(array.typeToggle[0], array.lang_menu[0] and u8'Полное умение' or 'Full skills', array.show_imgui_fullskills)
				...
				imgui.ToggleButton(array.typeToggle[0], array.lang_menu[0] and u8'Колокол' or 'Bell', array.bell)
                imgui.EndChild()
            elseif act1 == 4 then
				...
				imgui.ToggleButton(array.typeToggle[0], array.lang_menu[0] and u8'Поле зрения' or 'FOV changer', array.show_imgui_FOV)
				imgui.SameLine()
				imgui.Text(fa.ICON_COG)
				if imgui.IsItemClicked() then
				...
				if imgui.BeginPopup(array.lang_menu[0] and u8'Настройки FOV' or 'Settings FOV') then
					imgui.PushItemWidth(250)
					imgui.SliderFloat(array.lang_menu[0] and u8'Значение' or 'Value', array.FOV_value, 70.0, 108.0, '%.1f', 0.5)
					imgui.PopItemWidth()
					imgui.EndPopup()
				end
				imgui.ToggleButton(array.typeToggle[0], array.lang_menu[0] and u8'Быстрый телепорт' or 'Blink', array.show_imgui_blink)
				...
				if imgui.BeginPopup(array.lang_menu[0] and u8'Настройки Blink' or 'Settings Blink') then
					imgui.PushItemWidth(250)
					imgui.SliderFloat(array.lang_menu[0] and u8'Дистанция' or 'Distance', array.blink_dist, 1.0, 150.0, '%.1f', 1.5)
					imgui.PopItemWidth()
					imgui.EndPopup()
				end
				...
				imgui.ToggleButton(array.typeToggle[0], array.lang_menu[0] and u8'Быстрая карта' or 'Quick map', array.show_imgui_quickMap)
				...
				imgui.ToggleButton(array.typeToggle[0], array.lang_menu[0] and u8'Исправление чувствительности' or 'Sensetivity fix', array.show_imgui_sensfix)
				imgui.ToggleButton(array.typeToggle[0], array.lang_menu[0] and u8'Перезаход' or 'Reconnect', array.show_imgui_reconnect)
				...
				imgui.ToggleButton(array.typeToggle[0], array.lang_menu[0] and u8'Чистрый скриншот' or 'Clear screenshot', array.show_imgui_clrScr)
				imgui.SameLine()
				imgui.Text(fa.ICON_COG)
				if imgui.IsItemClicked() then
				...
				imgui.ToggleButton(array.typeToggle[0], array.lang_menu[0] and u8'Езда/Ходьба под водой' or 'Drive/Walk under water', array.show_imgui_UnderWater)
				imgui.NextColumn()
				imgui.ToggleButton(array.typeToggle[0], 'Rvanka', array.rvanka)
				imgui.TextQuestion('##rvanka', array.lang_menu[0] and u8'Функция не сохраняется\nБета' or 'Function don\'t save\nBeta')
				imgui.ToggleButton(array.typeToggle[0], array.lang_menu[0] and u8'Сразу отключить чат на F7' or 'Immediately disable chat on F7', array.offChatF7)
				imgui.ToggleButton(array.typeToggle[0], array.lang_menu[0] and u8'Деньги без задержки' or 'Cash without delay', array.notDelayCash)
				imgui.TextQuestion('##notDelayCash', array.lang_menu[0] and u8'Когда прибавляются или убавляются деньги, это происходит без задержки в HUD' or 'When cash is added or subtracted, it happens without delay in the HUD')
				imgui.ToggleButton(array.typeToggle[0], array.lang_menu[0] and u8'Гравитация' or 'Gravitation', array.gravitation)
				imgui.SameLine()
				imgui.Text(fa.ICON_COG)
				if imgui.IsItemClicked() then
					imgui.OpenPopup(array.lang_menu[0] and u8'Настройки Gravitation' or 'Settings Gravitation')
				end
				if imgui.BeginPopup(array.lang_menu[0] and u8'Настройки Gravitation' or 'Settings Gravitation') then
					imgui.PushItemWidth(250)
					imgui.SliderFloat(array.lang_menu[0] and u8'Значение' or 'Value', array.gravitation_value, 0.01, 0.0001, '%.4f', 1.0)
					imgui.PopItemWidth()
					imgui.EndPopup()
				end
				imgui.ToggleButton(array.typeToggle[0], array.lang_menu[0] and u8'Помощник ввода' or 'Input helper', array.inputHelper)
				imgui.TextQuestion('##inputhelper', array.lang_menu[0] and u8'Независимо от языка, если первый символ будет "." (заменится на "/") или "/",\nбудет вводится команда на английском языке' or 'Regardless of the language, if the first character is "." (will be replaced with "/") or "/",\nthe command will be entered in English')
				imgui.ToggleButton(array.typeToggle[0], array.lang_menu[0] and u8'Чат на "T"' or 'Chat on "T"', array.chatOnT)
				imgui.ToggleButton(array.typeToggle[0], array.lang_menu[0] and u8'Выбор строки клавишей' or 'Select a line with the key', array.fastSelectOnDialog)
				imgui.TextQuestion('##fastSelectOnDialog', array.lang_menu[0] and u8'При диалоге вы можете нажать на 1 (и до 0), чтоб выбрать строку' or 'During dialogue, you can click on 1 (and up to 0) to select a line')
				imgui.Columns(1, id, false)
				imgui.NewLine()
				imgui.CenterTextColored(imgui.ImVec4(imgui.GetStyle().Colors[imgui.Col.Text]), array.lang_menu[0] and u8'Телепорты' or 'Teleports')
            ...
            elseif act1 == 5 then
				...
				imgui.BeginTitleChild(array.lang_menu[0] and u8'Общее' or 'General', imgui.ImVec2(240, array.typeToggle[0] == 0 and 186 or 165))
				imgui.ToggleButton(array.typeToggle[0], array.lang_menu[0] and u8'Информационная панель' or 'Information bar', array.infbar)
				imgui.SameLine()
				imgui.Text(fa.ICON_COG)
				if imgui.IsItemClicked() then
				...
				if imgui.BeginPopup(array.lang_menu[0] and u8'Настройки Информационная панель' or 'Settings Information bar') then
					imgui.ToggleButton(array.typeToggle[0], array.lang_menu[0] and u8'Гравитация' or 'Gravitation', array.infbar_grav)
					imgui.ToggleButton(array.typeToggle[0], array.lang_menu[0] and u8'Координаты' or 'Coordinates', array.infbar_coords)
					imgui.ToggleButton(array.typeToggle[0], array.lang_menu[0] and u8'Интерьер' or 'Interior', array.infbar_interior)
					imgui.ToggleButton(array.typeToggle[0], array.lang_menu[0] and u8'Время' or 'Time', array.infbar_time)
					imgui.ToggleButton(array.typeToggle[0], array.lang_menu[0] and u8'Пинг' or 'Ping', array.infbar_ping)
					imgui.ToggleButton(array.typeToggle[0], 'FPS', array.infbar_fps)
					imgui.ToggleButton(array.typeToggle[0], 'ID', array.infbar_aid)
					imgui.ToggleButton(array.typeToggle[0], array.lang_menu[0] and u8'Транспорт ID' or 'Vehicle ID', array.infbar_vid)
					imgui.EndPopup()
				end
				imgui.ToggleButton(array.typeToggle[0], array.lang_menu[0] and u8'Информационная панель 2' or 'Information bar 2', array.infbar2)
				imgui.ToggleButton(array.typeToggle[0], array.lang_menu[0] and u8'Пульсатор денег' or 'Pulsator cash', array.pulsCash)
				imgui.ToggleButton(array.typeToggle[0], array.lang_menu[0] and u8'Помощник чата' or 'Chat helper', array.chatHelper)
				imgui.ToggleButton(array.typeToggle[0], array.lang_menu[0] and u8'Поиск 3D текстов' or 'Search 3D text', array.srch3dtext)
				imgui.ToggleButton(array.typeToggle[0], array.lang_menu[0] and u8'Трейсер пуль' or 'Traser bullets', array.traserbull)
				imgui.ToggleButton(array.typeToggle[0], array.lang_menu[0] and u8'Уведомления' or 'Notifications', array.notifications)
				imgui.SameLine()
				imgui.Text(fa.ICON_COG)
				if imgui.IsItemClicked() then
				...
				if imgui.BeginPopup(array.lang_menu[0] and u8'Настройки Уведомления' or 'Settings Notifications') then
					imgui.ToggleButton(array.typeToggle[0], array.lang_menu[0] and u8'GM персонаж' or 'GM actor', array.NactorGM)
					imgui.ToggleButton(array.typeToggle[0], array.lang_menu[0] and u8'GM транспорт' or 'GM vehicle', array.NvehGM)
				...
				imgui.EndChild()
				imgui.SameLine()
				imgui.BeginTitleChild(array.lang_menu[0] and u8'Игроки' or 'Players', imgui.ImVec2(160, array.typeToggle[0] == 0 and 91 or 85))
				imgui.ToggleButton(array.typeToggle[0], array.lang_menu[0] and u8'По клавише' or 'On key', array.show_imgui_keyWH)
				...
				imgui.ToggleButton(array.typeToggle[0], array.lang_menu[0] and u8'Скелет' or 'Skeleton', array.show_imgui_skeleton)
				imgui.EndChild()
				imgui.Spacing()
				imgui.BeginTitleChild(array.lang_menu[0] and u8'Транспорт' or 'Vehicle', imgui.ImVec2(240, array.typeToggle[0] == 0 and 66 or 65))
				imgui.ToggleButton(array.typeToggle[0], array.lang_menu[0] and u8'Только колеса' or 'Only wheels', array.show_imgui_onlyWheels)
				imgui.ToggleButton(array.typeToggle[0], array.lang_menu[0] and u8'Проверка дверей' or 'Check doors', array.show_imgui_doorlocks)
				imgui.SameLine()
				imgui.Text(fa.ICON_COG)
				if imgui.IsItemClicked() then
            ...
            elseif act1 == 6 then
				...
				if array.lang[0] == 3 then
					...
					if imgui.BeginPopup(array.lang_menu[0] and u8'Настройки Пользовательский' or 'Settings Custom') then
						imgui.ToggleButton(array.typeToggle[0], array.lang_menu[0] and u8'Меню' or 'Menu', array.lang_menu)
						imgui.ToggleButton(array.typeToggle[0], array.lang_menu[0] and u8'Чат' or 'Chat', array.lang_chat)
						imgui.ToggleButton(array.typeToggle[0], array.lang_menu[0] and u8'Визуалы' or 'Visual', array.lang_visual)
						imgui.Text('* ENG <-> RUS')
						imgui.EndPopup()
					end
				end
				imgui.TextColoredRGB(array.lang_menu[0] and u8'{0984d2}Выберите тип переключателей: ' or '{0984d2}Select the type of toggle: '); imgui.SameLine()
				...
				imgui.SameLine()
				imgui.ToggleButton(array.typeToggle[0], array.lang_menu[0] and u8'Авто-сохранение' or 'Auto-save', array.AutoSave)
				imgui.Spacing()
				if imgui.Button(array.lang_menu[0] and u8'Выгрузка' or 'Unload') then thisScript():unload() end
				imgui.IntSpacing(3)
				...
				imgui.Text(array.lang_menu[0] and u8'* Кликните по тексту для скопирования ссылки в буфер обмена' or '* Click on the text to copy the link to the clipboard')
				imgui.IntSpacing(3)
				imgui.ToggleButton(array.typeToggle[0], array.lang_menu[0] and u8'Проверка обновлений' or 'Check updates', array.checkupdate)
				imgui.SameLine()
				imgui.Indent(300)
				...
				if listupdate then
					...
					if imgui.BeginPopupModal('##listupdate', false, imgui.WindowFlags.NoTitleBar + imgui.WindowFlags.NoResize) then
						...
						imgui.Text('26.05.2021 - v1.6 '..(array.lang_menu[0] and u8'(Текущая)' or '(Current)'))
							imgui.Text('- '..(array.lang_menu[0] and u8'Убран префикс в командах, "/z_*", осталось только "/z*" (* - команда)' or 'Removed prefix in commands, "/z_*", only "/z*" is left (* - command)'))
							imgui.Text('- '..(array.lang_menu[0] and u8'Убрана команда "/zcmdsamp" и добавлены все команды которые там были во вкладку "Помощь"' or 'Removed the command "/zcmdsamp" and added all the commands that were there in the "Help" tab'))
							imgui.Text('- '..(array.lang_menu[0] and u8'Убрана возможность менять язык с диалогов (так как диалогов (локальных) в скрипте больше нет)' or 'Removed the ability to change the language from dialogs (since there are no more dialogues (local) in the script)'))
							imgui.Text('- '..(array.lang_menu[0] and u8'Теперь в команде "/zsetmark" можно указать аргументы: 1 - X, 2 - Y, 3 - Z, 4 - Интерьер (если не указан, то интерьер "0")' or 'Now in the command "/zsetmark" you can specify the arguments: 1 - X, 2 - Y, 3 - Z, 4 - Interior (if not specified, then interior "0")'))
							imgui.Text('- '..(array.lang_menu[0] and u8'Добавлена команда "/ztpgta", которая телепортирует на метку поставленную игроком' or 'Added the command "/ztpgta", which will teleport to the mark placed by the player'))
							imgui.Text('- '..(array.lang_menu[0] and u8'Изменена "Информационная панель", теперь настройки проще' or 'Changed "Information bar", now their settings are easier'))
							imgui.Text('- '..(array.lang_menu[0] and u8'Добавлены "Информационная панель 2", "Пульсатор денег", "Помощник чата" во вкладку "Визуалы"' or 'Added "Information bar 2", "Pulsator cash", "Chat helper" to the "Visual" tab'))
							imgui.Text('- '..(array.lang_menu[0] and u8'Добавлен выбор трех видов переключателей в "Настройки"' or 'Added selection of three types of switches in "Settings"'))
							imgui.Text('- '..(array.lang_menu[0] and u8'Добавлена "Быстро останавливаться" во вкладку "Персонаж"' or 'Added "Stop quickly" to the "Actor" tab'))
							imgui.Text('- '..(array.lang_menu[0] and u8'Добавлены "Сразу отключить чат на F7", "Деньги без задержки", "Гравитация", "Чат на "T"", "Выбор строки клавишей" во вкладку "Прочее"' or 'Added "Immediately disable chat on F7", "Cash without delay", "Gravitation", "Chat on "T"", "Select a line with the key" to the "Misc" tab'))
							imgui.Text('- '..(array.lang_menu[0] and u8'Прочие изменения и исправления' or 'Other changes and fixes'))
							imgui.NewLine()
						imgui.Text('18.04.2021 - v1.5')
							imgui.Text('- '..(array.lang_menu[0] and u8'Теперь на кнопку "ESCAPE" - можно закрыть меню' or 'Now on the "ESCAPE" button - you can close the menu'))
							imgui.Text('- '..(array.lang_menu[0] and u8'Теперь шрифт иконок не нужен, он есть в самом скрипте' or 'Now the icon font is not needed, it is in the script itself'))
							imgui.Text('- '..(array.lang_menu[0] and u8'Убраны подвкладки из вкладок' or 'Removed sub-tabs from tabs'))
							imgui.Text('- '..(array.lang_menu[0] and u8'Убрана функция "Транспорт отлетает если в него стрельнуть" из вкладки "Транспорт"' or 'Removed the function "Vehicle float away when hit" from the "Vehicle" tab'))
							imgui.Text('- '..(array.lang_menu[0] and u8'Убрана команда "/z_errors", убраны всевозможные ошибки из меню во вкладке "Помощь", также убрана нумерация ошибок' or 'Removed the command "/z_errors", removed all kinds of errors from the menu in the "Help" tab, also removed the numbering of errors'))
							imgui.Text('- '..(array.lang_menu[0] and u8'Добавлено "Быстрое меню" под всеми вкладками' or 'Added "Quick Menu" under all tabs'))
							imgui.Text('- '..(array.lang_menu[0] and u8'Добавлено "Rvanka" во вкладку "Разное"' or 'Added "Rvanka" to the "Misc" tab'))
							imgui.Text('- '..(array.lang_menu[0] and u8'Добавлена "Невидимка" во вкладку "Транспорт" и "Персонаж"' or 'Added "Invisible" to the tab "Vehicle" and "Actor"'))
							imgui.Text('- '..(array.lang_menu[0] and u8'Добавлен "Колокол" во вкладку "Оружие"' or 'Added "Bell" to the "Weapon" tab '))
							imgui.Text('- '..(array.lang_menu[0] and u8'Добавлена "Радуга" и возможность изменить цвет (кроме прозрачности) для функции "Поиск" для серверов' or 'Added functions for Russian servers'))
							imgui.Text('- '..(array.lang_menu[0] and u8'Оптимизация, исправлены ошибки' or 'Optimization, bugs fixed'))
							imgui.NewLine()
						imgui.Text('07.03.2021 - v1.4.1')
							imgui.Text('- ' .. (array.lang_menu[0] and u8'Исправлено авто-обновление' or 'Fixed auto-update'))
							imgui.Text('- ' .. (array.lang_menu[0] and u8'Удалена команда "/z_update"' or 'Removed command "/z_update"'))
							imgui.NewLine()
				...
				if imgui.Button(array.lang_menu[0] and u8'Проверить обновления' or 'Check updates') then autoupdate(jsn_upd, tag, url_upd) end
				imgui.SameLine()
				imgui.Indent(300)
				if imgui.CollapsingHeader(array.lang_menu[0] and u8'Инструменты для разработчиков' or 'Tools for developers') then
					imgui.ToggleButton(array.typeToggle[0], array.lang_menu[0] and u8'Информация диалогов в консоли SF' or 'Dialog info in SF console', array.dev_dialogid)
					imgui.ToggleButton(array.typeToggle[0], array.lang_menu[0] and u8'Отображать ID Textdraw' or 'Display ID Textdraw', array.dev_textdraw)
					imgui.ToggleButton(array.typeToggle[0], array.lang_menu[0] and u8'Информация GameText в консоли SF' or 'GameText info in SF console', array.dev_gametext)
					imgui.ToggleButton(array.typeToggle[0], array.lang_menu[0] and u8'Информация анимаций в консоли SF' or  'Animations info in SF console', array.dev_anim)
				end
				imgui.Unindent(300)
                imgui.EndChild()
            ...
            elseif act1 == 8 then
				...
				if imgui.CollapsingHeader(array.lang_menu[0] and u8'Команды' or 'Commands') then
					imgui.TextColoredRGB('{0984d2}/zhelp {ffffff}- ' .. (array.lang_menu[0] and u8'Помощь по скрипту' or 'Script help'))
					imgui.TextColoredRGB('{0984d2}/zdate {ffffff}- ' .. (array.lang_menu[0] and u8'Сегодняшняя дата' or 'Todays date'))
					imgui.TextColoredRGB('{0984d2}/zmenu {ffffff}- ' .. (array.lang_menu[0] and u8'{008900}Открытие/Закрытие {ffffff}меню' or '{008900}Open/Close {ffffff}menu'))
					imgui.TextColoredRGB('{0984d2}/zcoord {ffffff}- ' .. (array.lang_menu[0] and u8'Отмечает Ваши координаты' or 'You are coords'))
					imgui.TextColoredRGB('{0984d2}/zgetmoney {ffffff}- ' .. (array.lang_menu[0] and u8'Выдает 1.000$ (Визуально)' or 'Give cash 1.000$ (Visual)'))
					imgui.TextColoredRGB('{0984d2}/ztime {ffffff}- ' .. (array.lang_menu[0] and u8'Поменять время' or 'Change time'))
					imgui.TextColoredRGB('{0984d2}/zweather {ffffff}- ' .. (array.lang_menu[0] and u8'Поменять погоду' or 'Change weather'))
					imgui.TextColoredRGB('{0984d2}/zsetmark {ffffff}- ' .. (array.lang_menu[0] and u8'Поставить метку' or 'Create mark'))
					imgui.TextColoredRGB('{0984d2}/ztpmark {ffffff}- ' .. (array.lang_menu[0] and u8'Телепортироваться к метке' or 'Teleport to mark'))
					imgui.TextColoredRGB('{0984d2}/zcc {ffffff}- ' .. (array.lang_menu[0] and u8'Очистка чата' or 'Clear chat'))
					imgui.TextColoredRGB('{0984d2}/zchecktime {ffffff}- ' .. (array.lang_menu[0] and u8'Точное время по МСК' or 'Exact time by MSK'))
					imgui.TextColoredRGB('{0984d2}/zsuicide {ffffff}- ' .. (array.lang_menu[0] and u8'Суицид (Если в транспорте, то взрывает транспорт. Если пешком, то убивает Вас)' or 'Suicide (If you are in vehicle, then boom vehicle. If walking, it kills you)'))
					imgui.TextColoredRGB('{0984d2}/zreload {ffffff}- ' .. (array.lang_menu[0] and u8'Перезагружает данный скрипт' or 'Reload this is script'))
					imgui.TextColoredRGB('{0984d2}/zfps {ffffff}- ' .. (array.lang_menu[0] and u8'Выводит FPS' or 'Displays FPS'))
					imgui.TextColoredRGB('{0984d2}/ztpgta {ffffff}- ' .. (array.lang_menu[0] and u8'Телепортирует к поставленной метке' or 'Teleports to the placed mark'))
					if getServer('revent') then
						imgui.TextColoredRGB(u8'{0984d2}/ztogall {ffffff}- {008900}Выключает/Включает {ffffff}все чаты которые возможно (Для Revent-RP)')
						imgui.TextColoredRGB(u8'{0984d2}/zrepair {ffffff}- Чинит Ваш транспорт и пишет, что якобы Вы починились в "починке" (Для Revent-RP)')
					else
						imgui.TextColoredRGB('{0984d2}/zrepair {ffffff}- ' .. (array.lang_menu[0] and u8'Чинит транспорт' or 'Fixed vehicle'))
					end
				end
				if imgui.CollapsingHeader(array.lang_menu[0] and u8'Стандартные команды SA:MP' or 'Standart commands SA:MP') then
					imgui.TextColoredRGB('{0984d2}/headmove {ffffff}- ' .. (array.lang_menu[0] and u8'{008900}Включает/Выключает {ffffff}поворот головы' or '{008900}On/Off {ffffff}head rotation'))
					imgui.TextColoredRGB('{0984d2}/timestamp {ffffff}- ' .. (array.lang_menu[0] and u8'{008900}Включает/Выключает {ffffff}время возле каждого сообщения' or '{008900}On/Off {ffffff}time near each message'))
					imgui.TextColoredRGB('{0984d2}/pagesize {ffffff}- ' .. (array.lang_menu[0] and u8'Устанавливает количество строк в чате' or 'Set the number of lines in the chat'))
					imgui.TextColoredRGB('{0984d2}/fontsize {ffffff}- ' .. (array.lang_menu[0] and u8'Устанавливает толщину чата' or 'Set wide in the chat'))
					imgui.TextColoredRGB('{0984d2}/quit (/q) {ffffff}- ' .. (array.lang_menu[0] and u8'Быстрый выход из игры' or 'Quick exit from the game'))
					imgui.TextColoredRGB('{0984d2}/save [комментарий] {ffffff}- ' .. (array.lang_menu[0] and u8'Сохранение координат в {008900}savedposition.txt' or 'Save coordinates to {008900}savedposition.txt'))
					imgui.TextColoredRGB('{0984d2}/fpslimit {ffffff}- ' .. (array.lang_menu[0] and u8'Устанавливает лимит кадров в секунду' or 'Set the frames per second limit'))
					imgui.TextColoredRGB('{0984d2}/dl {ffffff}- ' .. (array.lang_menu[0] and u8'{008900}Включает/Выключает {ffffff}подробную информацию о транспорте по близости' or '{008900}On/Off {ffffff}detailed information about near vehicle'))
					imgui.TextColoredRGB('{0984d2}/interior {ffffff}- ' .. (array.lang_menu[0] and u8'Выводит в чат текущий интерьер' or 'Current interior to chat'))
					imgui.TextColoredRGB('{0984d2}/rs {ffffff}- ' .. (array.lang_menu[0] and u8'Сохранение координат в {008900}rawposition.txt' or 'Save coordinates to {008900}rawposition.txt'))
					imgui.TextColoredRGB('{0984d2}/mem {ffffff}- ' .. (array.lang_menu[0] and u8'Отображает сколько памяти использует SA:MP' or 'How much memory SA:MP uses'))
					imgui.NewLine()
				end
				imgui.TextColoredRGB(array.lang_menu[0] and u8'{008900}Авторство:\n{0984d2}PanSeek {ffffff}- Создатель\n{0984d2}Cosmo {ffffff}& {0984d2}FYP {ffffff}& {0984d2}AppleThe {ffffff}& {0984d2}hnnssy {ffffff}& {0984d2}deddosouru {ffffff}& {0984d2}SR_team {ffffff}- Исходный код/Помощь\n{0984d2}qrlk {ffffff}& {0984d2}DonHomka {ffffff}- Исходный код/Помощь\n{0984d2}Warhhogg {ffffff}& {0984d2}frannya {ffffff}- Спасибо за идеи' or '{008900}Authorship:\n{0984d2}PanSeek {ffffff}- Creator\n{0984d2}Cosmo {ffffff}& {0984d2}FYP {ffffff}& {0984d2}AppleThe {ffffff}& {0984d2}hnnssy {ffffff}& {0984d2}deddosouru {ffffff}& {0984d2}SR_team {ffffff}- Source/Helped\n{0984d2}qrlk {ffffff}& {0984d2}DonHomka {ffffff}- Source/Helped\n{0984d2}Warhhogg {ffffff}& {0984d2}frannya {ffffff}- Thanks for the ideas')
            ...
            elseif act1 == 10 then
				...
				imgui.ToggleButton(array.typeToggle[0], u8'Эмуляция лаунчера', array.arz_launcher)
				imgui.ToggleButton(array.typeToggle[0], u8'Быстрый /rep(ort)', array.arz_fastreport)
				...
				imgui.ToggleButton(array.typeToggle[0], u8'Пропуск диалогового окна при ответе администрации на Ваш /report', array.arz_autoskiprep)
				imgui.ToggleButton(array.typeToggle[0], u8'Поиск объектов', array.arz_venable)
				imgui.SameLine()
				imgui.Text(fa.ICON_COG)
				if imgui.IsItemClicked() then
				...
				if imgui.BeginPopup('Настройки Поиск ARZ') then
					...
					imgui.ToggleButton(array.typeToggle[0], u8'Радуга', array.arz_rainbow)
					imgui.SameLine()
					imgui.PushItemWidth(50)
					imgui.SliderInt(u8'Скорость', array.arz_speed, 1, 30)
					imgui.PopItemWidth()
					imgui.ToggleButton(array.typeToggle[0], u8'Рисовать линию', array.arz_vline)
					imgui.Separator()
					imgui.ToggleButton(array.typeToggle[0], u8'Оружия', array.arzsearchGuns)
					imgui.ToggleButton(array.typeToggle[0], u8'Семена', array.arzsearchSeed)
					imgui.ToggleButton(array.typeToggle[0], u8'Олени', array.arzsearchDeer)
					imgui.ToggleButton(array.typeToggle[0], u8'Наркотики', array.arzsearchDrugs)
					imgui.ToggleButton(array.typeToggle[0], u8'Подарки', array.arzsearchGift)
					imgui.ToggleButton(array.typeToggle[0], u8'Клады', array.arzsearchTreasure)
					imgui.ToggleButton(array.typeToggle[0], u8'Материалы', array.arzsearchMats)
					imgui.EndPopup()
				end
            ...
            elseif act1 == 9 then
				...
				imgui.ToggleButton(array.typeToggle[0], u8'Поиск объектов', array.rv_venable)
				...
				if imgui.BeginPopup('Настройки Поиск RVRP') then
					...
					imgui.SameLine()
					imgui.Text(u8'Общий цвет')
					imgui.ToggleButton(array.typeToggle[0], u8'Радуга', array.rv_rainbow)
					imgui.SameLine()
					imgui.PushItemWidth(50)
					imgui.SliderInt(u8'Скорость', array.rv_speed, 1, 30)
					imgui.PopItemWidth()
					imgui.ToggleButton(array.typeToggle[0], u8'Рисовать линию', array.rv_line)
					imgui.Separator()
					imgui.ToggleButton(array.typeToggle[0], u8'Трупы', array.rvsearchCorpse)
					imgui.ToggleButton(array.typeToggle[0], u8'Подковы', array.rvsearchHorseshoe)
					imgui.ToggleButton(array.typeToggle[0], u8'Тотемы', array.rvsearchTotems)
					imgui.ToggleButton(array.typeToggle[0], u8'Контейнеры', array.rvsearchCont)
					imgui.TextQuestion('##cont', u8'Могут быть не семейные контейнеры')
					imgui.EndPopup()
				end
				imgui.ToggleButton(array.typeToggle[0], u8'Исправление чата', array.rv_fixchat)
				imgui.TextQuestion('##rv_fixchat', u8'БЕТА!\nНа чат теперь более приятно смотреть')
				imgui.NewLine()
				imgui.CenterTextColored(imgui.ImVec4(imgui.GetStyle().Colors[imgui.Col.Text]), u8'Телепорты')
                ...
                actEnterInGame = 1
                imgui.BeginChild('##helpzuwi', imgui.ImVec2(sw-140, sh-40), true)
...
function funcs()
	...
	local EngineText = array.show_imgui_engineOnVeh[0] and (array.lang_visual[0] and '{29C730}Двигатель' or '{29C730}Engine') or (array.lang_visual[0] and '{B22C2C}Двигатель' or '{B22C2C}Engine')
...
-- VISUAL
	...
	if isPlayerPlaying(PLAYER_HANDLE) and not isPauseMenuActive() then
		...
		if array.chatHelper[0] and sampIsChatInputActive() then
			...
			local caps, num, numLets, clrNumLets = array.lang_visual[0] and (capsState == 1 and '{00FF00}Вкл' or '{FF0000}Выкл') or capsState == 1 and '{00FF00}On' or '{FF0000}Off', array.lang_visual[0] and (numState == 1 and '{00FF00}Вкл' or '{FF0000}Выкл') or numState == 1 and '{00FF00}On' or '{FF0000}Off', #sampGetChatInputText() == 0 and '0' or #sampGetChatInputText(), #sampGetChatInputText() >= 128 and '{FF0000}' or '{F9D82F}'
			local text = string.format('%s: {F9D82F}%s {CCC8C0}| CapsLock: %s {CCC8C0}| NumLock: %s {CCC8C0}| %s: %s%d/128 {CCC8C0}| {F9D82F}%s {CCC8C0}| {F9D82F}%s', array.lang_visual[0] and 'Язык' or 'Language', string.match(localName, "([^%(]*)"), caps, num, array.lang_visual[0] and 'Символы' or 'Symbols', clrNumLets, numLets, sampGetPlayerNickname(select(2, sampGetPlayerIdByCharHandle(PLAYER_PED)))..'['..select(2, sampGetPlayerIdByCharHandle(PLAYER_PED))..']' , os.date('%X'))
		...
		if array.infbar[0] then
			...
			local grav, tCoords, tInterior, tFps, ping, aid, vid = string.format(array.lang_visual[0] and '[Гравитация: {F9D82F}%.4f{888EA0}]' or '[Gravitation: {F9D82F}%.4f{888EA0}]', mem.getfloat(0x863984)), string.format('[X: {F9D82F}%.1f {888EA0}Y: {F9D82F}%.1f {888EA0}Z: {F9D82F}%.1f{888EA0}]' or '', playerX, playerY, playerZ), string.format(array.lang_visual[0] and '[Интерьер: {F9D82F}%d{888EA0}]' or '[Interior: {F9D82F}%d{888EA0}]', getActiveInterior()), string.format('[FPS: {F9D82F}%d{888EA0}]', mem.getfloat(0xB7CB50, 4, false)), string.format(array.lang_visual[0] and '[Пинг: {F9D82F}%d{888EA0}]' or '[Ping: {F9D82F}%d{888EA0}]', sampGetPlayerPing(select(2, sampGetPlayerIdByCharHandle(PLAYER_PED)))), string.format('[ID: {F9D82F}%d{888EA0}]', select(2, sampGetPlayerIdByCharHandle(PLAYER_PED))), isCharInAnyCar(PLAYER_PED) and string.format('[VID: {F9D82F}%d{888EA0}]', select(2, sampGetVehicleIdByCarHandle(storeCarCharIsInNoSave(PLAYER_PED))))
...
-- SERVERS
	local rv_tabl = {
		...
		{array.rvsearchCont[0], 19321, 'Контейнер'}, -- mb miss :/
		{array.rvsearchCont[0], 2932, 'Контейнер'},
		{array.rvsearchCont[0], 2934, 'Контейнер'}
	}
	local arz_tabl = {
...
function sampev.onSendGiveDamage()
	if array.bell[0] then
		local audio = loadAudioStream('moonloader/resource/zuwi/bell.mp3')
...
function onScriptTerminate(zuwiScript, quitGame)
	if zuwiScript == thisScript() then
		...
		sampAddChatMessage(tag..(array.AutoSave[0] and (array.lang_chat[0] and 'Скрипт аварийно закончил работу. Настройки сохранены' or 'The script crashed. Settings have been saved') or (array.lang_chat[0] and 'Скрипт аварийно закончил работу' or 'The script crashed')), colors.chat.main)
	end
end
				...
				if f then
					...
					elseif updateversion > version_script then
						lua_thread.create(function(prefix)
							local dlstatus = require('moonloader').download_status
							sampAddChatMessage(tag..(array.lang_chat[0] and 'Доступно обновление! Скачиваю {0E8604}свежую {888EA0}версию {F9D82F}'..updateversion or 'An update is available. Downloading {0E8604}the latest {888EA0}version '..updateversion), colors.chat.main)
							wait(250)
							downloadUrlToFile(updatelink, thisScript().path, function(id3, status1, p13, p23)
								if status1 == dlstatus.STATUS_DOWNLOADINGDATA then
									log('Downloading')
								elseif status1 == dlstatus.STATUS_ENDDOWNLOADDATA then
									sampAddChatMessage(tag..(array.lang_chat[0] and 'Скачивание завершено. Скрипт обновлен на версию {F9D82F}'..updateversion or 'The download is complete. The script has been updated to version '..updateversion), colors.chat.main)
									goupdatestatus = true
